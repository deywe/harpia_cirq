...
# Diretório para logs (igual ao seu exemplo anterior)
...
def entrada_parametros():
    try:
        num_qubits = int(input("🔢 Número de Qubits no circuito GHZ (Cirq): "))
...
def gerar_ghz_circuit_com_ruido(num_qubits: int, noise_prob: float = None):
    ...
    if noise_prob is None:
        try:
            noise_prob = float(input("Digite o nível de ruído depolarizante (0.0 a 1.0): "))
            noise_prob = max(0.0, min(noise_prob, 1.0))  # Garante que está no intervalo [0, 1]
        except ValueError:
            print("Valor inválido. Usando noise_prob=0.01 como padrão.")
            noise_prob = 1.91
...
# Executável externo ⎯ mantém o mesmo do seu script original
...
def gerar_uid_via_bscore():
    try:
        ...
        print(f"❌ Erro ao executar UID Rust: {e}")
        return "-", 0.0, "Erro"

# Loop principal
def executar_simulacao(num_qubits, total=100, noise_prob=0.01):
    print("=" * 60)
    print(f"    ☯️ HARPIA QGHZ (CIRQ) • {num_qubits} Qubits • {total:,} Frames")
    print("=" * 60)
    ...
    for frame in tqdm(range(1, total + 1), desc="⏳ Simulando GHZ Cirq"):
        ...
        print(f"Frame {frame}: Res={resultado} | UID={uid_val} | B(t)={bscore_val:.4f} | Status={status_uid} | ✓={'✅' if aceito else '❌'}")
        sys.stdout.flush()
    ...
    plt.scatter(
        ...
        c=['green' if row[8] == "✅" else 'red' for row in log_data],
        s=8, alpha=0.6,
