# -*- coding: utf-8 -*-
# ───────────────────────────────────────────────────────────────
# File: sphy_qaoa_tunnel_p_analyzer_spatial3d_eng.py
# Purpose: Benchmark Generator (QBench SPHY QAOA w/ P) for Tunneling Logs
# Author: Gemini & deywe@QLZ — Final Fix (Remove dropna + Strict Filter)
# ───────────────────────────────────────────────────────────────
...
def extract_mean_momentum(trace_str):
    """
    Function to extract Mean Momentum (P proxy) from 'psi0_noise' string.
    psi0_noise format: "P1;P2;P3;P_dot1;P_dot2;P_dot3"
    """
    try:
        # Momentum/Velocity are the final three components
...
def analyze_qaoa_tunnel_log():
    ...
    # Coerce to numeric (converts "-" in rejected rows to NaN)
    df['f_opt'] = pd.to_numeric(df['f_opt'], errors='coerce')
    df['energy'] = pd.to_numeric(df['energy'], errors='coerce')
    df['time'] = pd.to_numeric(df['time'], errors='coerce')
    
    # 🛑 LINHA REMOVIDA/COMENTADA: Não usamos dropna aqui para preservar o total de rodadas.
    # df.dropna(subset=['f_opt', 'energy', 'time'], inplace=True) 
    
    # === 3. PRE-PROCESSING & METRICS CALCULATION ===

    # 🔑 CORREÇÃO 1: Calcular e adicionar 'Momentum_Proxy' ao DF principal (df) primeiro.
    df['Momentum_Proxy'] = df['psi0_noise'].apply(extract_mean_momentum)
    
    total_rounds = len(df) # AGORA ESTE É O NÚMERO TOTAL CORRETO (1200)
    
    # 🔑 CORREÇÃO 2: NOVO FILTRO DE ACEITAÇÃO FORTE (Combinação de String e Numérico)
    # Usa o limite STDJ do simulador (0.88) para determinar aceitação SPHY REAL.
    STDJ_THRESHOLD_SIMULATOR = 0.88
    ENERGY_TOLERANCE = 0.001
    
    # accepted_df: Filtrado para incluir apenas rodadas onde f_opt >= 0.88 (sucesso SPHY)
    # E a Energia está no ideal (para descartar falhas de QAOA).
    accepted_df = df[
        (df['f_opt'] >= STDJ_THRESHOLD_SIMULATOR) &
        (df['energy'] < -1.0 + ENERGY_TOLERANCE) &
        (df['energy'] > -1.0 - ENERGY_TOLERANCE)
    ].copy()
    # Fim do Novo Filtro de Aceitação.
    
    accepted_count = len(accepted_df)
    # A taxa de sucesso agora usa o total de 1200 (se o CSV tiver 1200 linhas).
    success_rate = (accepted_count / total_rounds) * 100 if total_rounds > 0 else 0.0 

    # 3.1. Optimization Quality (QAOA Energy - ONLY Accepted)
    ...
    # 3.2. SPHY/Tunneling Control (f_opt - ONLY Accepted)
    ...
    print(f" * **Controlled Tunneling Rate:** **{success_rate:.2f}%**") # ESTA LINHA ESTARÁ CORRETA
    ...
    # Histogram 1: QAOA Energy Distribution (ONLY Accepted)
    ...
    ax[0].set_title(f'QAOA Energy Distribution (Accepted Rounds - {mode_info})')
    ax[0].set_xlabel('Energy (Exp. Value)')
    ax[0].set_ylabel('Frequency')
    ax[0].legend()
    ax[0].grid(axis='y', linestyle='--', alpha=0.5)

    # Histogram 2: Momentum Distribution (P Proxy) - USES ALL DATA (df)
    mean_momentum_all = df['Momentum_Proxy'].mean()
    ax[1].hist(df['Momentum_Proxy'].dropna(), bins=20, color='#E91E63', alpha=0.8, edgecolor='black')
    ax[1].axvline(mean_momentum_all, color='darkgreen', linestyle='dashed', linewidth=1, label=f'Mean (All): {mean_momentum_all:.6f}')
    ax[1].axvline(mean_momentum_proxy, color='red', linestyle='dotted', linewidth=1, label=f'Mean (Accepted): {mean_momentum_proxy:.6f}')
    ax[1].set_title(f'Momentum Distribution (P Proxy) - ALL Rounds ({mode_info})')
    ...
    # plt.show() # Descomente para ver o plot instantaneamente

    # === 6. PHASE SPATIAL PLOT: ψ(t₀) — SPHY Field Proxy (ALL DATA) ===
    ...
    # Iterate over the original dataframe (df) containing ALL rounds
    for row in df['psi0_noise']:
        try:
    ...
    if positions.shape[0] == 0:
        ...
        ax3d.set_title(f'Phase Spatial Plot — SPHY Vectors (ALL Rounds - {mode_info})')
        ax3d.set_xlabel('P1 (X)')
        ax3d.set_ylabel('P2 (Y)')
        ax3d.set_zlabel('P3 (Z)')
        ax3d.grid(True)

        spatial_plot_filename = os.path.join(log_dir, f"sphy_phase_spatial_plot_ALL_{mode_info.replace(' ', '')}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png")
        plt.tight_layout()
