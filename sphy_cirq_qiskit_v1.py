...
def generate_ghz_state_cirq(num_qubits, noise_prob=1.0): #Bit-flip noise harder propositally!
    ...
    # ❗ Inserção de ruído no Cirq
    # O Cirq usa "canais" para ruído, que podem ser inseridos
...
def simulate_frame_cirq(frame_data):
    ...
    # ❗ Execução do circuito no Cirq
    # `run` executa o circuito e retorna um objeto `Result`
    result_cirq = simulator.run(circuit, repetitions=1)
    
    # ❗ Obter o resultado da medição
    # O Cirq retorna um array booleano. Precisamos converter para string binária.
